...
function ENT:Draw()
    ...
    if LocalPlayer():GetPos():DistToSqr(self:GetPos()) <= 360000 then
        ...
        cam.Start3D2D(pos, ang, 0.15) 
            ...
            if status == 1 then
                draw.SimpleTextOutlined('Coffre-fort', 'BankFont', 0, -640, color_white, 1, 1, 5, color_black)
                draw.SimpleTextOutlined('Braquage: '..string.ToMinutesSeconds(math.Clamp(math.Round(self:GetNextAction() -CurTime()), 0, BANK_CONFIG.RobberyTime)), 'BankFont', 0, -565, Color(255, 0, 0), 1, 1, 5, color_black)
            elseif status == 2 then
                draw.SimpleTextOutlined('Coffre-fort', 'BankFont', 0, -640, color_white, 1, 1, 5, color_black)
                draw.SimpleTextOutlined('Délai: '..string.ToMinutesSeconds(math.Clamp(math.Round(self:GetNextAction() -CurTime()), 0, BANK_CONFIG.CooldownTime)), 'BankFont', 0, -565, Color(255, 0, 0), 1, 1, 5, color_black)
            else
                draw.SimpleTextOutlined('Coffre-fort', 'BankFont', 0, -565, Color(255, 255, 255, 255), 1, 1, 5, color_black)
            end
        cam.End3D2D()
    ...
    if BANK_CONFIG.Government[team.GetName(LocalPlayer():Team())] then
        for k, v in pairs(ents.FindByClass('bank_vault')) do
            if v:GetStatus() == 1 then
                ...
                draw.DrawText('BANQUE EN COURS DE BRAQUAGE', 'Default', pos.x, pos.y, HSVToColor(CurTime() *100 %360, 1, 1), 1)
                break
            end
        end
    end
